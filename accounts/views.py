# accounts/views.py - COMPLETE MOBILE-OPTIMIZED VERSION
import json
import traceback
import base64
import requests
...
def update_profile(request):
    ...
    try:
        ...
        return JsonResponse({'success': False, 'error': error_dict}, status=400)
...
def login_view(request):
    ...
    return render(request, 'accounts/login.html', {'social_errors': social_errors})
    ...
    if request.method == 'POST':
        try:
            ...
            if friend_request:
                ...
                if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                    return JsonResponse({'success': True, 'message': f'Friend request to {to_user.username} cancelled.'})
...
def remove_friend(request, user_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({'success': True, 'message': f'{friend.username} removed from friends.'})
            ...
            if email_or_phone['type'] == 'email':
                ...
                try:
                    ...
                    if verification_sid:
                        ...
                        request.session['twilio_phone_number'] = phone
...
def force_verify_user(request):
    """Force verify current user - TEMPORARY"""
...
def verify_otp_setup(request):
    ...
    return render(request, 'accounts/otp_verify_setup.html')
...
def otp_status(request):
    ...
    return render(request, 'accounts/otp_status.html', context)
...
def verify_login_otp(request):
    ...
    if request.method == 'POST':
        ...
        if not otp_code:
            messages.error(request, 'Please enter OTP code.')
            return render(request, 'accounts/otp_verify_login.html')
